...
We can run the full test entire process with a single line. For a description of all these parameters see our other vignettes. The result is the Seurat object with the `OARscore`, `KW.pvalue`, `KW.BH.pvalue` and `sparsity` values in the `meta.data` slot.
...
To explore the results we can visualize `OARscore` vs `sparsity` values, coloring the cells based on some of the attributes in our Seurat object meta.data.
